...
let businessSwiper = new Swiper('.business .slide', {
    loop: true,
    slidesPerView: 1.2, /* 옆에 이미지가 살짝 보이게 */
    spaceBetween: 30,
...
gsap.timeline({
    scrollTrigger: {
        trigger: '.vid_box',
        start: 'top 10%',
        end: "top 10%+=1500", //1500px 스크롤 동안 애니메이션 진행
        scrub: 1,
        pin: '.vid_box', //고정
        pinSpacing: true,
...
gsap.timeline({
    scrollTrigger: {
        ...
        end: () => "+=" + (cards.length * 380), //카드 수 * 380만큼 더 내려가서 끝
        scrub: 1, //스크롤을 움직이면 애니메이션도 같이 부드럽게 따라가게
        invalidateOnRefresh: true, // 화면이 새로고침될 때 카드들의 위치를 다시 계산해줌
    }
}).fromTo(cards, { y: 300, z: 200, opacity: 0.8 }, { y: 0, z: 0, duration: 1, ease: "power2.out", stagger: 1, opacity: 1 })//stagger : 카드 하나씩 1초 간격으로 차례차례 등장 시키는 속성
    .to(cards, { y: -300, z: -200, skewY: 5, duration: 1, ease: "power2.in", stagger: 1, }, '+=0.5') //앞 애니메이션 끝나고 0.5초 쉬었다가 등장



let newSwiper = new Swiper('.news .con', {
    slidesPerView: 'auto',
    loop: true,
    autoplay: {
        delay: 2500,
